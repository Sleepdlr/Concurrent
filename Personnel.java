...
public abstract class Personnel extends Unit{
    ...
    public void startWar(List<Personnel> enemyArmy){
        while(this.health.get() > 0 && !enemyArmy.isEmpty()){
            this.opponent = enemyArmy.get(random.nextInt(enemyArmy.size()));
            startAttacking();
        }
    }
    ...
    private void startAttacking(){
        while(opponent.getHealth() > 0 && this.health.get() > 0){
            int attackValue = this.getAttack();
            opponent.loseHealth(attackValue);
